...
import requests
import json 
import recomendation as rec
import youtuberecomendation as yt
...
def index():
    ...
    if request.method == 'POST':
        ...
        msg = rec.searchds(x)
        return render_template('index.html', user = False, namehead = "Get Recomendations", videos = videos, msg = msg)
...
def predict():
    if request.method == 'POST':
        ...
        predict = rec.predict(userInput, request.form.get('o'))
        if current_user.is_authenticated:
            ...
            db.session.add(info)
            db.session.commit()
            videos = yt.youvideoauth(predict)
...
def admin():
    if request.method == 'POST':
        if request.form.get('name') == parameters["adminUsername"] and request.form.get('password') == parameters["adminPassword"] and request.form.get('email') == parameters["adminEmail"]:
